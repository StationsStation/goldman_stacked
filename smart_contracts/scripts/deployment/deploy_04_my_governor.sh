...
contractPath="smart_contracts/src/MyGovernor.sol:MyGovernor"
constructorArgs="$votingTokenAddress $timelockAddress $governorRelayLZAddress"
...
myGovernorAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#myGovernorAddress}
...
echo "$(jq '. += {"myGovernorAddress":"'$myGovernorAddress'"}' scripts/deployment/globals_base_mainnet.json)" > scripts/deployment/globals_base_mainnet.json
...
echo "Contract deployed at: $myGovernorAddress"